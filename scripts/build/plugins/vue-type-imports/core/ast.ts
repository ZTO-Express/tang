...
import type { MaybeAliases, Replacement, StringMap } from './utils'
import { getAst, groupImports, insertString, intersect, resolveModulePath } from './utils'
...
export function getAvailableImportsFromAst(ast: Program): GetImportsResult {
  ...
  for (const node of ast.body) {
    if (node.type === 'ImportDeclaration' && node.specifiers.length) addImport(node)
  }
...
export async function extractTypesFromSource(
  ...
  if (!isInternal) imports.push(...getAvailableExportsFromAst(ast))
  ...
  function getTSNodeMap(nodes: TSTypes[]): NodeMap {
    ...
    for (const node of nodes) {
      if ('name' in node.id) nodeMap.set(node.id.name, node)
    }
  ...
  function extractTypeByName(name: string) {
    ...
    if (extractedTypes.get(name)) return

    const node = nodeMap.get(name)

    if (node) ExtractTypeByNode(node)
    else missingTypes.push(name)
  }
  ...
  const extractTypesFromTSUnionType = (union: TSUnionType) => {
    union.types
      ...
      .forEach(typeReference => {
        if (typeReference.typeName.type === 'Identifier') extractTypeByName(typeReference.typeName.name)
      })
  }
  ...
  function extractExtendInterfaces(interfaces: TSExpressionWithTypeArguments[], interfaceMetaData: InterfaceMetaData) {
    for (const extend of interfaces) {
      if (extend.expression.type === 'Identifier') {
        ...
        if (isInternal) extraSpecifiers.push(name)
  ...
  const extractTypesFromInterface = (node: TSInterfaceDeclaration) => {
    ...
    if (extendInterfaceName) {
      ...
      if (isInternal && cleanInterface) removeInterface(node)
      ...
      if (extendsInterfaces) {
        if (isInternal) removeInterface(node)
  ...
  const extractTypesFromEnum = (node: TSEnumDeclaration) => {
    ...
    for (const member of node.members) {
      if (member.initializer) {
        if (member.initializer.type === 'NumericLiteral') enumTypes.add('number')
        else if (member.initializer.type === 'StringLiteral') enumTypes.add('string')
      } else {
  ...
  for (const typeName of types) extractTypeByName(typeName)
  ...
  if (missingTypes.length) {
    await Promise.all(
      Object.entries(groupImports(imports)).map(async ([modulePath, importedFields]) => {
        ...
        if (intersection.length) await extractTypesFromModule(modulePath, intersection)
      })
    )
  }
...
export function isTSTypes(node: MaybeNode, isInternal: boolean): node is TSTypes {
  if (isInternal) return !!(node && node.type === 'TSInterfaceDeclaration')
