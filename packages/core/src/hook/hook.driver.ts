import { sortBy, ensureArray } from '../utils/lodash'
import { throwInvalidHookError, throwHookError } from './hook-util'

import type { Hook } from './hook-util'
...
export class HookDriver<T> {
  ...
  // 异步并行执行钩子，忽略返回值
  hookParallel(hookName: string, context: T, args?: any[]): Promise<unknown | void> {
  ...
  hookSeq(hookName: string, context: T, args?: any): Promise<void> {
    ...
    for (let i = 0; i < this.hooks.length; i++) {
      promise = promise.then(() => this.runHook(hookName, context, args, i) as Promise<void>)
    }
    return promise
  }
