import { PageSchema } from 'zpage/core'
...
function getPagePathByPageKey(key: string) {
  ...
  let path = key.substr(basePath.length)

  if (path.endsWith(indexPostfix)) {
    path = path.substr(0, path.length - indexPostfix.length)
  } else if (path.endsWith('.ts')) {
    path = path.substr(0, path.length - '.ts'.length)
  }
